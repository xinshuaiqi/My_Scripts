...
LOOP1: while (<NAME>) {
	...
	if($load < 48) {
		...
		print "\n\nBlasting against AthBra.pro\n\n";
		system ("cd Translated/$taxon/; blastx -num_threads $CPU -evalue 0.01 -max_target_seqs 50 -db ~/bin/ATH.BRA.Proteins/AthBra.pro -query no_cl.$taxon.minlength300 -out out.blastx_$taxon"); # blasts your sequences against 25 plant genomes' protein sequences
		...
		system ("cd Translated/$taxon/; prot_fasta2.pl prot_ids1.clean.unique_col0.clean.blastxparsed.out.blastx_$taxon ~/bin/ATH.BRA.Proteins/AthBra.pro"); #output is prot_fasta.prot_ids1.clean.unique_col0.clean.blastxparsed.out.blastx_$taxon
